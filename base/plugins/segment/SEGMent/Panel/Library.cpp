...
{
  auto ressourcesPath = QFileInfo(qApp->applicationFilePath()).absolutePath() + "/Ressources";
...

  {
    ...
    lay->addWidget(new DraggableButton{tr("Click Area"), SEGMENT_CLICKAREA_ID, m_zones});
    lay->addWidget(new DraggableButton{tr("Back Click Area"), SEGMENT_BACKCLICKAREA_ID, m_zones});
    lay->addWidget(new DraggableButton{tr("Text Area"), SEGMENT_TEXTAREA_ID, m_zones});
    lay->addWidget(new QSplitter);

    lay->addWidget(new DraggableButton{tr("Text riddle"), SEGMENT_TEXTRIDDLE_ID, m_zones});
    lay->addWidget(new DraggableButton{tr("Gif riddle"), SEGMENT_GIFRIDDLE_ID, m_zones});
    lay->addWidget(new DraggableButton{tr("Puzzle riddle"), SEGMENT_PUZZLERIDDLE_ID, m_zones});
    lay->addStretch();
  ...
  {
    m_scenes = new ImageList(ressourcesPath, SCENE_IMAGES_DIRECTORY, SEGMENT_SCENE_ID);
  ...
  {
    m_objects = new ImageList(ressourcesPath, OBJECT_IMAGES_DIRECTORY, SEGMENT_OBJECT_ID);
  ...
  {
    m_templates = new TemplateList(ressourcesPath, TEMPLATES_DIRECTORY, SEGMENT_TEMPLATE_ID);
  ...
  {
    m_sounds = new SoundList(ressourcesPath, SOUNDS_DIRECTORY, SEGMENT_SOUND_ID);
    m_sounds->reloadList();
...
const score::PanelStatus& Library::defaultPanelStatus() const
{
  static const score::PanelStatus status{
    true, Qt::LeftDockWidgetArea, 10,
        QObject::tr("SEGMent Library"), QObject::tr("Ctrl+U")};
    ...
    score::MaybeDocument oldm, score::MaybeDocument newm)
{
  if(oldm)
  {
    disconnect(&oldm->document.metadata(), &score::DocumentMetadata::fileNameChanged,
               this, &Library::on_documentPathChanged);
  }
  if(newm)
  {
    connect(&newm->document.metadata(), &score::DocumentMetadata::fileNameChanged,
            this, &Library::on_documentPathChanged);
