...
public:
  ...
  virtual transition_t createTransitionFrom(anchor_id source_anchor, const SceneModel& target, anchor_id target_anchor) = 0;

  const std::vector<Anchor*>& anchors() const noexcept { return m_anchorsSetter.anchors(); }
protected:
  Window(
      QPointF pos,
      QSizeF sz,
...
class ObjectResizer final
    : public SizeGripItem::Resizer
{
public:
  using resize_command =
    typename score::PropertyCommand_T<
      typename std::remove_reference_t<
        decltype(std::declval<T>().model())
      >::p_size
  ...
  void operator()(QGraphicsItem* item, QRectF& rect) override
  {
    ...
    self.context.dispatcher.template submitCommand<resize_command>(self.model(), rel);
...
struct ObjectMover
{
    ...
    { return (event->buttons() & Qt::LeftButton) || (event->button() == Qt::LeftButton); }
    static bool isRight(QGraphicsSceneMouseEvent* event)
    { return (event->buttons() & Qt::RightButton) || (event->button() == Qt::RightButton); }
    ...
    static void move(T& self, QGraphicsSceneMouseEvent* event)
    {
      ...
      if(isLeft(event))
      {
        ...
        {
          self.setPos(prect.width() - self.boundingRect().width(), self.pos().y());
        }

        if (self.pos().y() + self.boundingRect().height() > prect.height())
        {
          self.setPos(self.pos().x(), prect.height() - self.boundingRect().height());
        }
        QPointF pt{ self.pos().x() / prect.width(), self.pos().y() / prect.height() };
        using command_t = typename score::PropertyCommand_T<
          typename std::remove_reference_t<
            decltype(std::declval<T>().model())
          >::p_pos
        >::template command<void>::type;
    ...
    static void release(T& self, QGraphicsSceneMouseEvent* event)
    {
      if(isLeft(event))
      {
        if(event->scenePos() != event->buttonDownScenePos(Qt::LeftButton))
        {
          ...
          using command_t = typename score::PropertyCommand_T<
            ...
            >::p_pos
          >::template command<void>::type;
          self.context.dispatcher.template submitCommand<command_t>(self.model(), pt);
          self.context.dispatcher.commit();
        }
        else
        {
      ...
      else if(isRight(event))
      {
        ...
        QObject::connect(act, &QAction::triggered, &self, [=,&self] {
          ...
          c.submitCommand(new RemoveObject{(const SEGMent::SceneModel&) *self.model().parent(), self.model()});
        });
...
struct ObjectDropper
{
    ...
    {
      using cmd = typename score::PropertyCommand_T<Prop>::template command<void>::type;
