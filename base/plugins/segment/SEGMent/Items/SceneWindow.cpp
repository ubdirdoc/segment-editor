...
class SceneResizer final
    : public SizeGripItem::Resizer
{
public:
  ...
  {
    auto new_rect = QRectF{
        self.pos().x(), self.pos().y(), rect.width(),
        self.rect().height() / self.rect().width() * rect.width()};
    auto rectItem = static_cast<SceneWindow*>(item);
    self.context.dispatcher.submitCommand<SEGMent::SetSceneRect>(self.model(), new_rect);
    rect = rectItem->rect();
  }
              ...
              , false, this, ctx, this}
{
  ...
  const auto set_titlebar_color = [&] (const SceneModel::SceneType t)
  {
    ...
    case SceneModel::SceneType::Default:
      m_titleBar.setBrush(default_color); break;
    case SceneModel::SceneType::Initial:
      m_titleBar.setBrush(initial_color); break;
    case SceneModel::SceneType::Final:
      m_titleBar.setBrush(final_color); break;
    case SceneModel::SceneType::GameOver:
      m_titleBar.setBrush(gameover_color); break;
    }
  };

  set_titlebar_color(p.sceneType());
  con(m_scene, &SceneModel::sceneTypeChanged,
      this, set_titlebar_color);
  ...
  con(m_title, &QLineEdit::editingFinished,
      this, [&] {
  ...
  QObject::connect(
      this, &RectItem::on_sizeChanged, this,
      &SceneWindow::sizeChanged);
  ...
  p.backClickAreas.removed.connect<&SceneWindow::on_backClickAreaRemoved>(this);
  ...
  ::bind(p.metadata(), score::ModelMetadata::p_label{}, this, [=] (auto img) { setTitle(img); });
...
{
  m_sceneArea.setRect(
        m_sceneArea.rect().x(), m_sceneArea.rect().y(),
  ...
  m_titleBar.setRect(
      m_titleBar.rect().x(), m_titleBar.rect().y(),
      m_sceneArea.rect().width(), m_titleBar.rect().height());
...
void SceneWindow::setBackgroundImage(QPixmap img)
{
  ...
  qreal fitRatio
      = sceneRect.width() / m_backgroundImgRealWidth;

  m_backgroundImgDisplay.setScale(fitRatio);

  m_sceneArea.setRect(
      m_backgroundImgDisplay.pos().x(), m_backgroundImgDisplay.pos().y(),
...
void SceneWindow::dropEvent(QGraphicsSceneDragDropEvent* event)
{
  ...
  QSizeF sz{1./5., 1. / 5.};
  QPointF pos{ localPos.x() / this->boundingRect().width(), localPos.y() / this->boundingRect().height() };
  ...
  if(currentMimeData->hasUrls())
  {
    if (currentMimeData->text() == SEGMENT_OBJECT_ID)
    {
      ...
      if(QFileInfo(currentUrl.fileName()).completeSuffix().compare("gif", Qt::CaseInsensitive) == 0)
      {
        disp.submitCommand(new DropGif{m_scene, Image{currentUrl.path()}, pos, sz});
      }
      else
      {
        disp.submitCommand(new DropImage{m_scene, Image{currentUrl.path()}, pos, sz});
      }
    }
...
QVariant SceneWindow::itemChange(GraphicsItemChange change, const QVariant &value)
{
...
void SceneWindow::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
  if(m_moving)
  {
    ...
    context.dispatcher.submitCommand<SetSceneRect>(
          m_scene,
          QRectF{
            qBound(-max_dim, this->pos().x(), max_dim),
...
void renderGraphicsItemRec(QPainter& painter, QGraphicsItem& root, QGraphicsItem& item, QWidget* view)
{
...
{
  QImage p(item.boundingRect().width(), item.boundingRect().height(), QImage::Format::Format_ARGB32);
  p.fill(Qt::white);
  {
    QPainter painter{&p};
    QStyleOptionGraphicsItem opt;
...
QString imageToBase64(const QImage& image)
{
  ...
  return QString::fromLatin1(byteArray.toBase64(QByteArray::Base64Encoding).data());
}
  ...
  if(event->button() == Qt::RightButton)
  {
    auto m = new QMenu;
    {
      ...
      connect(act, &QAction::triggered, [=] {
        ...
        c.submitCommand(new RemoveScene{(const SEGMent::ProcessModel&) *m_scene.parent(), m_scene});
      });
    }
      ...
      connect(act, &QAction::triggered, [=] {
        ...
        json_object["Pixmap"]
            = imageToBase64(
                renderGraphicsItem(
                  *this,
                  this->scene()->views().first()
                  ).scaled(100, 100)
                );

        // Save in a file in the library
        auto ressources = ressourcesFolder(context);
        auto template_file = addUniqueSuffix(ressources + "/Templates/" + m_scene.metadata().getLabel() + ".template");
