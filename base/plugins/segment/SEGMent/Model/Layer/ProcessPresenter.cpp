...
Presenter::Presenter(
    ...
    QObject* parent)
    : QObject{parent}, context{ctx}, m_view{*view}
{
...
void Presenter::on_transitionCreated(const TransitionModel& transition)
{
  ...
  struct tm
  {
    ...
    ret operator()(const GifToScene& t) const { return {nullptr, nullptr}; }
    ...
    ret operator()() const { return {nullptr, nullptr}; }
  } transition_matcher{*this};

  auto [start, end]
      = ossia::apply(transition_matcher, transition.transition());
  if (start && end)
  {
