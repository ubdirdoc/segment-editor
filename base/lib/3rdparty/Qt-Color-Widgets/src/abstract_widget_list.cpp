    ...
    : QWidget(parent), p(new Private)
{
  ...
  connect(
      &p->mapper_down, SIGNAL(mapped(QWidget*)), SLOT(down_clicked(QWidget*)));
  connect(
      &p->mapper_remove,
  ...
  QPushButton* add_button
      = new QPushButton(QIcon::fromTheme("list-add"), tr("Add New"));
...
void AbstractWidgetList::appendWidget(QWidget* w)
{
  ...
  QWidget* b_down
      = create_button(w, &p->mapper_down, "go-down", tr("Move Down"));
  QWidget* b_remove
      = create_button(w, &p->mapper_remove, "list-remove", tr("Remove"));
  if (row == 0)
...
QWidget* AbstractWidgetList::create_button(
    QWidget* data,
