...
public:
  ...
  bool
  save(ColorPalette& palette, const QString& suggested_filename = QString())
  {
    ...
    QRegularExpression name_regex(
        QRegularExpression::escape(palette.name()) + "([0-9]+)\\.gpl");
    int max = 0;
    ...
    return attemptSave(
        ...
        save_dir.absoluteFilePath(
            QString("%1%2.gpl").arg(palette.name()).arg(max + 1)));
  }
};
...
bool ColorPaletteModel::removeRows(
    int row,
    int count,
...
bool ColorPaletteModel::updatePalette(
    int index,
