...
public:
   ...
   */
  QPixmap
  preview(const QSize& size, const QColor& background = Qt::transparent) const;
   ...
   */
  void
  insertColor(int index, const QColor& color, const QString& name = QString());
  W_SLOT(insertColor)
  /**
   ...
   */
  void colorsChanged(const QVector<QPair<QColor, QString>>& p)
      W_SIGNAL(colorsChanged, p);
   ...
   * (set, append etc.)
   */
  void colorsUpdated(const QVector<QPair<QColor, QString>>& c)
      W_SIGNAL(colorsUpdated, c);
  ...
  W_PROPERTY(
      QVector<value_type>,
      colors READ colors WRITE setColors NOTIFY colorsChanged)
  /**
