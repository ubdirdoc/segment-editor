...
public:
  ...
  void colorChanged(const QColor& color) W_SIGNAL(colorChanged, color)
      /**
       * \brief Emitted when the user is typing a color but has not finished
       * yet
       */
      ...
      void colorEditingFinished(const QColor& color)
          W_SIGNAL(colorEditingFinished, color)

              void showAlphaChanged(bool showAlpha) W_SIGNAL(
                  showAlphaChanged,
                  showAlpha) void previewColorChanged(bool previewColor)
                  W_SIGNAL(previewColorChanged, previewColor)

                      W_PROPERTY(
                          QColor,
                          color READ color WRITE setColor NOTIFY colorChanged)
      /**
       * \brief Whether the widget displays and edits the alpha channel
       */
      W_PROPERTY(
          bool,
          showAlpha READ showAlpha WRITE setShowAlpha NOTIFY showAlphaChanged)
      /**
      ...
      W_PROPERTY(
          bool,
          previewColor READ previewColor WRITE setPreviewColor NOTIFY
              previewColorChanged)
